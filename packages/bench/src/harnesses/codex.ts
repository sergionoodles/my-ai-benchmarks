  ...
  async run(prompt, workdir, model, timeoutSec) {
    ...
    if (process.env.BENCH_LIVE_HARNESS !== "1") {
      return {
        ...
        costUsd: null,
        log: `$ codex exec --model ${model} [STUB: set BENCH_LIVE_HARNESS=1 for a real run]\nPrompt was ${prompt.length} chars; workspace left untouched.`,
      };
    }
    ...
    try {
      ...
      return { status: "ok", durationMs: Date.now() - started, costUsd: null, log: `$ codex exec --model ${model}\n${stdout}\n${stderr}` };
    } catch (err) {
